...
#define RETURN_SUCCESS(format, ...) do { cunitMessage(__FUNCTION__, __FILE__, __LINE__, format, ## __VA_ARGS__); return SUCCESS; } while (0)
#define FAIL(format, ...) do { cunitMessage(__FUNCTION__, __FILE__, __LINE__, "***FAILURE*** " format, ## __VA_ARGS__); total_failures++; goto failure; } while (0)
