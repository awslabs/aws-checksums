...
* Computes the Castagnoli CRC32c (iSCSI) of the specified data buffer using the Intel CRC32Q (quad word) machine instruction
* by operating on 24-byte stripes in parallel. The results are folded together using CLMUL.
* This function is optimized for exactly 256 byte blocks that are best aligned on 8-byte memory addresses.
...
* Private (static) function.
* Computes the Castagnoli CRC32c (iSCSI) of the specified data buffer using the Intel CRC32Q (quad word) machine instruction
* by operating on 3 24-byte stripes in parallel. The results are folded together using CLMUL.
* This function is optimized for exactly 1024 byte blocks that are best aligned on 8-byte memory addresses.
...
* Computes the Castagnoli CRC32c (iSCSI) of the specified data buffer using the Intel CRC32Q (quad word) machine instruction
* by operating on 24-byte stripes in parallel. The results are folded together using CLMUL.
* This function is optimized for exactly 3072 byte blocks that are best aligned on 8-byte memory addresses.
...
* Pass 0 in the previousCrc32 parameter as an initial value unless continuing to update a running CRC in a subsequent call.
*/
